...
GreedyBestFirstPlanner::GreedyBestFirstPlanner(std::shared_ptr<Heuristic> heuristic)
    : heuristic(std::move(heuristic))
{
}

std::string GreedyBestFirstPlanner::name() const
{
    return "Greedy Best First Search [" + heuristic->name() + "]";
}

std::list<GroundedAction> GreedyBestFirstPlanner::plan(const Env &env, SearchStats *stats) const
{
    ...
    *stats = SearchStats();
    ...
    best_g[start.key] = 0;
    open.push({heuristic->estimate(start.state, env), 0, 0, sequence++});
    ...
    while (!open.empty())
    {
        ...
        for (const GroundedOperator &op : operators)
        {
            ...
            open.push({h, next_g, child_index, sequence++});
